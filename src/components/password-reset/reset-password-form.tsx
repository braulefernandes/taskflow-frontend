...
function resetError(error: unknown) {
  if (isApiError(error) && error.code === "invalid_reset_token") return "Este link e inválido, expirou ou já foi utilizado. Solicite uma nova redefinição.";
...
export function ResetPasswordForm() {
  ...
  if (!token) return <div className="mt-8"><div className="rounded-lg border border-red-200 bg-red-50 p-4 text-sm text-red-800" role="alert">Token de redefinição ausente. Solicite um novo link.</div><p className="mt-6 text-center text-sm"><Link className="font-medium underline" href="/recuperar-senha">Solicitar novo link</Link></p></div>;

  return <form className="mt-8 space-y-5" noValidate onSubmit={handleSubmit((values) => mutation.mutate(values))}>{mutation.isSuccess ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800" role="status">Senha redefinida com sucesso. Redirecionando para o login.</div> : null}<ErrorMessage message={mutation.error ? resetError(mutation.error) : undefined} /><FormField id="new-password" label="Nova senha" hint="Use de 8 a 128 caracteres, com letras e números." error={errors.newPassword?.message}><Input autoComplete="new-password" disabled={mutation.isPending || mutation.isSuccess} id="new-password" type="password" {...register("newPassword")} /></FormField><FormField id="password-confirmation" label="Confirmação da senha" error={errors.passwordConfirmation?.message}><Input autoComplete="new-password" disabled={mutation.isPending || mutation.isSuccess} id="password-confirmation" type="password" {...register("passwordConfirmation")} /></FormField><Button className="w-full" disabled={mutation.isPending || mutation.isSuccess} type="submit">{mutation.isPending ? "Redefinindo..." : "Redefinir senha"}</Button><p className="text-center text-sm"><Link className="font-medium underline" href="/login">Voltar ao login</Link></p></form>;
}
