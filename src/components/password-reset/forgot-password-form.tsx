...
export function ForgotPasswordForm() {
  ...
  return <div className="mt-8">{mutation.isSuccess ? <div className="rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800" role="status">{genericMessage}</div> : <form className="space-y-5" noValidate onSubmit={handleSubmit((values) => mutation.mutate(values))}><ErrorMessage message={mutation.error ? "Não foi possível enviar as instrucoes agora. Tente novamente." : undefined} /><FormField id="recovery-email" label="E-mail" error={errors.email?.message}><Input autoComplete="email" disabled={mutation.isPending} id="recovery-email" inputMode="email" type="email" {...register("email")} /></FormField><Button className="w-full" disabled={mutation.isPending} type="submit">{mutation.isPending ? "Enviando..." : "Enviar instrucoes"}</Button></form>}<p className="mt-6 text-center text-sm"><Link className="font-medium text-slate-950 underline" href="/login">Voltar ao login</Link></p></div>;
}
