...
async function openAndFillForm() {
  ...
  fireEvent.change(screen.getByLabelText("Senha temporaria"), { target: { value: "Temporaria123" } });
}
...
describe("gerenciamento de membros", () => {
  ...
  it("permite que administrador acesse a pagina", async () => { setup(); expect(await screen.findByRole("heading", { name: "Membros" })).toBeDefined(); });
  it("bloqueia visualmente outro papel e nao consulta o backend", () => { vi.mocked(useSession).mockReturnValue(session("MANAGER")); setup(); expect(screen.getByRole("alert").textContent).toContain("Apenas administradores"); expect(listMembers).not.toHaveBeenCalled(); });
