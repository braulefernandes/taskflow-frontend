...
function errorMessage(error: unknown) {
  if (isApiError(error) && error.code === "last_active_admin") return "A organização deve manter ao menos um administrador ativo.";
  if (isApiError(error) && error.code === "membership_already_exists") return "Este e-mail já pertence a organização.";
...
function MembersManagement() {
  ...
  return <section aria-labelledby="members-title" className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold" id="members-title">Membros</h1><p className="mt-1 text-sm text-slate-600">Gerencie o acesso da sua organização.</p></div><Button onClick={() => setFormOpen(!formOpen)} type="button">{formOpen ? "Fechar formulário" : "Novo membro"}</Button></header>
    ...
    {query.isError ? <div className="rounded-xl border border-red-200 bg-red-50 p-6" role="alert"><p>Não foi possível carregar os membros.</p><Button className="mt-3" onClick={() => query.refetch()} type="button">Tentar novamente</Button></div> : null}
