...
describe("ações operacionais da solicitação", () => {
  ...
  it("não oferece estado operacional enquanto ticket pendente está sem responsável", () => { const pending = { ...ticket, status: "PENDING" as const, assignee: null }; setup("ADMIN", pending); expect(screen.queryByRole("heading", { name: "Alterar status" })).toBeNull(); expect(validTicketTransitions(pending, "ADMIN", "admin")).toEqual([]); });
  ...
  it("solicitante só recebe cancelamento para ticket próprio pendente", () => { const pending = { ...ticket, status: "PENDING" as const, assignee: null }; const own = setup("REQUESTER", pending, "requester"); expect(screen.getByRole("button", { name: "Cancelar solicitação" })).toBeDefined(); own.unmount(); setup("REQUESTER", pending, "outro"); expect(screen.queryByRole("button", { name: "Cancelar solicitação" })).toBeNull(); });
  it("atualiza detalhe e inválida cache", async () => { const { client, invalidate } = setup(); fireEvent.click(screen.getByRole("button", { name: "Aguardando retorno" })); await screen.findByText("Status atualizado com sucesso."); expect(client.getQueryData(["tickets", "detail", "ticket-1"])).toEqual(ticket); expect(invalidate).toHaveBeenCalled(); });
