...
function apiError(error: unknown) {
  if (isApiError(error) && error.code === "category_already_exists") return "Já existe uma categoria com este nome.";
  if (error instanceof TypeError) return "Não foi possível conectar ao servidor.";
  return "Não foi possível concluir a operacao. Tente novamente.";
}
...
function CategoriesManagement() {
  ...
  return <section aria-labelledby="categories-title" className="space-y-6">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-2xl font-bold" id="categories-title">Categorias</h1><p className="mt-1 text-sm text-slate-600">Organize os tipos de solicitação da sua organização.</p></div><Button onClick={() => setEditing(null)} type="button">Nova categoria</Button></header>
    ...
    <div className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 sm:grid-cols-2"><label className="text-sm font-medium">Buscar<Input aria-label="Buscar categorias" className="mt-1" onChange={(event) => setSearch(event.target.value)} placeholder="Nome ou descrição" value={search} /></label><label className="text-sm font-medium">Status<select aria-label="Filtrar por status" className="mt-1 block min-h-10 w-full rounded-lg border border-slate-300 px-3" onChange={(event) => setStatus(event.target.value as StatusFilter)} value={status}><option value="all">Todos</option><option value="active">Ativas</option><option value="inactive">Inativas</option></select></label></div>
    {query.isPending ? <div className="rounded-xl border bg-white p-10"><Loading label="Carregando categorias" /></div> : null}
    {query.isError ? <div className="rounded-xl border border-red-200 bg-red-50 p-6" role="alert"><p>Não foi possível carregar as categorias.</p><Button className="mt-3" onClick={() => query.refetch()} type="button">Tentar novamente</Button></div> : null}
