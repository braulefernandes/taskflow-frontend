...
export function ProfilePage() {
  ...
  if (!session) return <section className="rounded-xl border border-amber-200 bg-amber-50 p-6" role="alert"><h1 className="text-xl font-bold">Sessao indisponivel</h1><p className="mt-2 text-sm">Entre novamente para visualizar seu perfil.</p></section>;

  return <section aria-labelledby="profile-title" className="mx-auto max-w-3xl space-y-6">
    <header><h1 className="text-2xl font-bold text-slate-950" id="profile-title">Meu perfil</h1><p className="mt-1 text-sm text-slate-600">Atualize seus dados pessoais. Dados administrativos sao somente leitura.</p></header>
    ...
    <form className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6" noValidate onSubmit={handleSubmit((values) => mutation.mutate(values))}>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <ReadOnly id="profile-organization" label="Organizacao" value={session.organization.name} />
        <ReadOnly id="profile-role" label="Papel" value={roleLabels[session.membership.role]} />
      </div>
      <div className="mt-6 flex justify-end"><Button disabled={mutation.isPending || !isDirty} type="submit">{mutation.isPending ? "Salvando..." : "Salvar alteracoes"}</Button></div>
    </form>
  </section>;
}
