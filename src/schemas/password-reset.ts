...
export const forgotPasswordSchema = z.object({
  email: z.string().trim().toLowerCase().min(1, required).email("Informe um e-mail válido.").max(320),
});
...
}).refine((values) => values.newPassword === values.passwordConfirmation, { path: ["passwordConfirmation"], message: "As senhas informadas não coincidem." });
